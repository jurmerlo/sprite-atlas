...
export class Image {
   ...
   */
  static fromBuffer(buffer: ArrayBuffer, trim: boolean, extrude: number): Image {
    const pngData = PNG.sync.read(Buffer.from(buffer));

    return new Image({ width: pngData.width, height: pngData.height, pngData, trim, extrude });
  }
   ...
   * @param extrude The number of pixels to extrude the image borders by.
   */
  constructor({ width, height, pngData, trim = false, extrude = 0 }: ImageParams) {
  ...
  extrudeEdges(amount: number): void {
    const original = new Image({ width: this.width, height: this.height, pngData: this._pngData });
  ...
  private trimTransparentPixels(): readonly [number, number] {
    const temp = new Image({ width: this.width, height: this.height, pngData: this._pngData });
